...
// RenderConsent sends a text-based consent prompt (Zalo OA has no inline buttons).
...
func (a *Adapter) HandleConsentText(senderID, text string) bool {
	...
	if a.ownerUserID != "" && senderID != a.ownerUserID {
		log.Printf("zalo: consent response from non-owner %s (expected %s)", senderID, a.ownerUserID)
